...
<div class="prose max-w-none space-y-4 text-center dark:prose-invert">
    {#if result !== null}
        {@const { sim, strength } = result}
        <section>
            <h1>SIM Information</h1>
            {#if sim === null}
                <Error>Unable to retrieve SIM information.</Error>
            {:else}
                {@const { network_type, carrier_id, carrier_name, operator_id, operator_name } = sim}
                <DisplaySim
        ...
        {#if strength === null}
            <Error>Unable to retrieve cellular signal strengths.</Error>
        {:else}
